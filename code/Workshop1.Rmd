...
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) #"l" added to displ
```
...
```{r}
ggplot() 
# Or with just the data specified?   #Nothing comes up on the plot, its blank
ggplot(data = mpg)
```
...
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy, colour = class)) #geom_point add layers of points to your plot, takes mapping argument which paired with aes() 
```
...
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy))   #got rid ot "x" in displx
```
...
ggplot(data = mpg) + 
  ...
  geom_smooth(mapping = aes(x = displ, y = hwy)) # try smooth line    #Error, have to get rid of "s" in mappings
```
...
```{r}
ggplot(data = mpg) + 
  geom_point(mapping = aes(x = displ, y = hwy)) +  #Error, so have to ger rid of repeated "x" in "xx"
  geom_smooth(mapping = aes(x = displ, y = hwy))
```
...
We specify and tell ggplot that the y axis should be a frequency or instructing ggplot to use the provided frequency values for the y-axis rather than computing counts automatically.
